...
def flow_translate_untranslated_target_ai(storage: GlossStorage, state: dict):
    """
    Translate native-language glosses that lack target-language translations.
    """
    ...
    for ref in candidate_refs:
        ...
        if gl and gl.language == state["native_language"]:
            glosses.append(gl)
    if not glosses:
        message_dialog(title="Info", text="No native glosses need target translations.").run()
        return

    ai_note = get_ai_note(state["target_language"])
    results: list[tuple[str, str, list[str]]] = []
    with ProgressBar(title="Translating to target language") as pb:
        for gl in pb(glosses):
            try:
                translations = translate_gloss(api_key, gl.content, state["target_language"], ai_note=ai_note)
            except Exception as exc:  # noqa: BLE001
    ...
    for ref, content, translations in results:
        ...
        for t_text in selection:
            try:
                t_gloss = storage.ensure_gloss(state["target_language"], t_text)
