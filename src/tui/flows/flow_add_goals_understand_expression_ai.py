...
GOALS_SYSTEM_PROMPT = "You create understand-expression goals in the target language."
GOALS_USER_PROMPT = (
    "Generate {num} understand-expression-goals in {target_language} for the situation: \"{situation_content}\". "
...
def generate_procedural_goals(api_key: str, situation_content: str, target_language: str, num: int) -> list[str]:
...
def flow_add_goals_procedural_paraphrase_ai(storage: GlossStorage, state: dict) -> None:
    ...
    try:
        goals = generate_procedural_goals(
            api_key=api_key,
    ...
    for goal_text in selection:
        ...
        if not goal_text:
            continue
        existing = storage.find_gloss_by_content(state["target_language"], goal_text)
        if existing:
            ...
            goal = storage.create_gloss(
                Gloss(content=goal_text, language=state["target_language"], tags=["eng:understand-expression-goal"])
            )
            created += 1
