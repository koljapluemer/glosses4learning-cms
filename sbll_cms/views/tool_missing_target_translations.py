...
@tools_bp.route("/missing-target-translations/manual", methods=["GET", "POST"])
def missing_target_translations_manual():
    storage = get_storage()
    ...
    if request.method == "POST" and action == "mark_impossible":
        ...
        return redirect(url_for("tools.missing_target_translations_manual", refs=refs_raw, native_language=native_language, target_language=target_language))

    return render_template(
        "tool_missing_target_translations/manual.html",
        glosses=glosses,
        refs_json=refs_raw,
...
@tools_bp.route("/missing-target-translations/input", methods=["GET"])
def missing_target_translations_input():
    refs_raw = request.values.get("refs") or "[]"
    native_language = (request.values.get("native_language") or "").strip().lower()
    target_language = (request.values.get("target_language") or "").strip().lower()
    languages = get_language_store().list_languages()
    return render_template(
        "tool_missing_target_translations/input_form.html",
        refs_json=refs_raw,
        native_language=native_language,
        target_language=target_language,
