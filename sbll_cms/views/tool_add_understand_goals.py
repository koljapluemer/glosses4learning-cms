...
def _generate_understand_goals_openai(
    api_key: str,
    model: str,
    ...
    """Generate understand-expression-goals in target language."""
    prompt = f"""Generate {num_goals} understand-expression-goals in {target_language} for the situation: "{situation_content}".

These are target language expressions that learners should understand in this situation.
Return practical, common phrases that a learner would encounter in this context.

{ai_note}

Additional context: {context}

Return a JSON object with a 'goals' array containing the {target_language} expressions."""
        ...
        response = requests.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a language learning curriculum designer who creates practical learning goals for language learners."
                    },
...
@tools_bp.route("/add-understand-goals/<language>/<slug>/input", methods=["GET"])
def add_understand_goals_input(language: str, slug: str):
    ...
    return render_template(
        "tool_add_understand_goals/input_form.html",
        situation=situation,
...
@tools_bp.route("/add-understand-goals/<language>/<slug>/output", methods=["GET", "POST"])
def add_understand_goals_output(language: str, slug: str):
        ...
        if json_mode:
            if action in ("ai_generate", "generate"):
                ...
                if not settings or not settings.api_keys.openai:
                    ...
                    ai_error = "Target language is required."
                else:
                    # Get language AI note if available
                    ai_note = ""
                    lang_store = get_language_store()
                    target_lang_obj = lang_store.get(target_language)
                    if target_lang_obj and hasattr(target_lang_obj, "ai_note"):
                        ai_note = target_lang_obj.ai_note or ""

                    # Generate goals
                    result = _generate_understand_goals_openai(
    ...
    return render_template(
        "tool_add_understand_goals/output_form.html",
        situation=situation,
        target_language=target_language,
        native_language=native_language,
...
@tools_bp.route("/add-understand-goals/<language>/<slug>/manual", methods=["GET", "POST"])
def add_understand_goals_manual(language: str, slug: str):
    """Route 3: Batch manual creation of understand-expression-goals."""
    storage = get_storage()
    situation = storage.load_gloss(language, slug)
    if not situation:
        flash("Situation not found.", "error")
    ...
    return render_template(
        "tool_add_understand_goals/manual.html",
        situation=situation,
        target_language=target_language,
        native_language=native_language,
        message=message,
