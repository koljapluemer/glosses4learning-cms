...
These relationships must be to glosses within the same language.
This is enforced in the editing UI by simply not allowing to select a language.
...
- `children`: Used to model relationships that are not necessarily linguistically derived, but didactically useful. E.g. "counting to three" may have *the children* "one", "two", "three". 
